...
class Message extends Component {
    ...
    createMessHeader = (item, index) => {
        ...
        // 处理消息时间 - 规则：每一分钟显示一次时间
        if(index === 0) {
            prevTime = new Date(item.created_at);
            return <div key={index} className="message-time"><span>{friendTimeShow(item.created_at)}</span></div>
        }
    ...
    createMessTimeDom = (createdTime, index) => {
        const endTime   = new Date(createdTime); // 结束时间
